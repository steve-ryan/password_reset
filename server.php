    ...
    if(!empty($_POST['event']) &&!empty($_POST['venue']) && !empty($_POST['date'])){
        ...
        $date = mysqli_real_escape_string($db_connection, htmlspecialchars($_POST['date']));
            ...
            if(mysqli_num_rows($check_event) > 0){    
                ...
                echo "<h3>This event already exist. Please Try another.</h3>";
                
            }else{
                
                // INSERT EVENT DATA INTO THE DATABASE
                $insert_query = mysqli_query($db_connection,"INSERT INTO `task`(name,venue, event_date) VALUES('$event','$venue','$date')");
